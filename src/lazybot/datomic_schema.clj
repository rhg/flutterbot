(ns lazybot.datomic-schema
  {:from "https://github.com/Datomic/day-of-datomic/blob/master/src/datomic/samples/schema.clj"}
      ...
      (let [{:keys [requires txes]} (get schema-map schema-name)]
        ...
        (if txes
          (doseq [tx txes]
            ...
            @(d/transact conn (vector {:db/id #db/id [:db.part/tx]
                                    schema-attr schema-name}
                                   (conj tx [:db/id #db/id [:db.part/db]]))))
          #_(throw (ex-info (str "No data provided for schema" schema-name)
                          {:schema/missing schema-name})))))))

(defn get-txs
  [schemas]
  (if schemas
    (for [[ident type cardinality doc ?more] schemas]
...
(defn load-schema
  [bot schema]
  (ensure-schemas (d/connect (:datomic/uri @bot))
                  :lazybot/schema
                  (:datomic/schema-map @bot)
                  schema))